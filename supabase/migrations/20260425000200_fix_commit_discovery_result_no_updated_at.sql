-- 20260425000200_fix_commit_discovery_result_no_updated_at.sql
-- discovery_runs has no updated_at column (only created_at). Task 16's
-- commit_discovery_result RPC wrote UPDATE discovery_runs SET updated_at = NOW()
-- which crashes with 42703 "column updated_at does not exist". Patch: drop the
-- updated_at assignment. completed_at already carries the timestamp signal.
-- Caught by the live smoke on 2026-04-25.
