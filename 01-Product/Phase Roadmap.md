...
- ✅ **Duplicate prevention hardened with regression-test ring + Gemini-enriched watchdog** (sync 17 / T18 + T19 + T20, 2026-04-26) — T18 centralized `_normalize_handle(handle, platform)` at `_commit_v2` layer (35 case-insensitive platforms); `_r` + `lang` tracking-param strips; `_classify_linkme_redirector` runs FIRST in `classify()` so `visit.link.me/...?sensitiveLinkLabel=OF` correctly classifies as `(onlyfans, monetization)` at conf 1.0. T19 added 13-test `test_commit_v2_dedup.py` + 2-test `test_platform_enum_drift.py` + `new_platform_watchdog` SQL view (migration `20260426060000`) + PROJECT_STATE §21 invariants section. T20 added 4 nullable TEXT columns to `classifier_llm_guesses` (migration `20260426070000`); LLM prompt now returns `suggested_label`/`suggested_slug`/`description`/`icon_category`; watchdog view replaced (migration `20260426080000`) to JOIN those columns so VAs see Gemini's recommendation per host. Data migration: 5 duplicate profile rows soft-deleted, 7 active rows handle-normalized, Kira's link.me OF redirector reclassified retroactively. **249 pytest + 1 skip. tsc 0.**
