...
Last synced: 2026-05-01 (sync 22 - harvester hardening, Shirley backfill, Apify-credit pause)
...
| `/creators/[slug]` | `(dashboard)/creators/[slug]/page.tsx` | 🟡 Partial — Brand HQ surface: stats strip, AvatarWithFallback (prefers `creators.override_avatar_url`, then best scraped profile avatar from any platform, including read-time fallback from recent `scraped_content.raw_apify_payload`), network sections with brand icons + account avatars, Add Account + Re-run Discovery in header, **DiscoveryProgress live bar** during in-flight runs, **`<BannerWithFallback>` client component** rendered below merge-candidate banner / above header (uses `creators.banner_url` override or `creators.cover_image_url` fallback — both nullable until Phase 3 scraper writes covers), **All Destinations section moved BELOW tabs (T16, 2026-04-26) wrapped in collapsed-by-default native `<details>` — still grouped by destination_class with `gated` chips on headless-harvested rows**, **AccountRow restructured (T17): Column 1 (180px) = `[icon] [Platform Name]`, Column 2 (280px) = avatar + handle (clickable) + display_name secondary line.** Bio + platform identity stripped from header (account-level data lives only in the network rows). Content/Branding/Funnel tabs are coming-soon stubs. |
...
| `/scraped-content` | `(dashboard)/scraped-content/page.tsx` + `components/content/ScrapedContentLibraryPage.tsx` | ✅ Live — reads `scraped_content` joined through `profiles.workspace_id`, shows latest non-rejected posts including IG `story_highlight` rows, deterministic copy-priority score, format filter, outlier/pinned/sponsored/quality flags, creator links, known-view coverage, view/engagement stats, audio metadata, repeat-audio post + creator counts, URL filters |
| `/trends` | `(dashboard)/trends/page.tsx` | ✅ Live — Audio Trends table from `trends WHERE trend_type='audio'`, filters by min usage/min creators/search/sort, shows usage count + distinct creator count, and links back to `/scraped-content?scope=trended` |
| `/admin` | `(dashboard)/admin/page.tsx` | ⬜ Placeholder |
| `/platforms/instagram/accounts` | `platforms/instagram/accounts/page.tsx` + `InstagramAccountsClient.tsx` | ✅ Live — real `profiles` query, profile photos from `profiles.avatar_url` or latest scraped payload fallback, stat cards, tracking tab URL filter, rank chip client filter, empty state, Unlinked badge |
| `/platforms/instagram/outliers` | `platforms/instagram/outliers/page.tsx` + `components/content/PlatformOutliersPage.tsx` | ✅ Live — platform-filtered outlier posts from `scraped_content`, excludes rejected rows, filters by format/scope/search/sort/min multiplier/audio trend, shows view coverage + repeat-audio creator counts + copy priority |
| `/platforms/instagram/classification` | — | ⬜ Placeholder |
| `/platforms/instagram/analytics` | — | ⬜ Placeholder |
| `/platforms/tiktok/accounts` | `platforms/tiktok/accounts/page.tsx` + `TikTokAccountsClient.tsx` | ✅ Live — real `profiles` query, profile photos from `profiles.avatar_url` or latest scraped payload fallback, stat cards, tracking tab URL filter, rank chip client filter, empty state, Unlinked badge |
| `/platforms/tiktok/outliers` | `platforms/tiktok/outliers/page.tsx` + `components/content/PlatformOutliersPage.tsx` | ✅ Live — platform-filtered outlier posts from `scraped_content`, excludes rejected rows, filters by format/scope/search/sort/min multiplier/audio trend, shows view coverage + repeat-audio creator counts + copy priority |

**Known cleanup:** Sidebar previously had duplicate tracking-type entries (Managed/Candidates/Competitors/Inspiration) that duplicated the in-page chip filter row — removed 2026-04-25 (commit `c75903d`). On 2026-04-28 the primary nav was trimmed to working product surfaces only; placeholder/Soon routes still exist but are hidden from deployable navigation until they have real workflows.
...
8. ✅ **Discovery v2 (SP1):** two-stage resolver (fetch seed → classify+enrich destinations); deterministic URL classifier with LLM fallback cache; rule-cascade identity scorer with CLIP avatar tiebreak; multi-platform fetcher layer (IG/TT via Apify, YT via yt-dlp, OF via curl_cffi, Patreon/Fanvue/generic via httpx, FB+X stubbed for SP1.1); `bulk_imports` as first-class observable job; cross-workspace identity dedup on every commit via persistent `profile_destination_links` index; Manual Add Account triggers full resolver expansion with canonical-field protection. 102 pytest tests. Spec: `docs/superpowers/specs/2026-04-24-discovery-v2-design.md`. Plan: `docs/superpowers/plans/2026-04-24-discovery-v2-plan.md`. ✅ **Recursive funnel resolver** (sync 14): bounded follow-until-terminus expansion through `_expand`. ✅ **Universal URL Harvester** (sync 15, 2026-04-26): replaces per-aggregator `linktree.py` / `beacons.py` / `custom_domain.py` extractors with a single `scripts/harvester/` package — Tier 1 httpx static fetch + 4-signal escalation detector → Tier 2 Apify Puppeteer Scraper headless render with `window.open` / `location.href` setter hooks installed pre-page-script + auto-click 7 interstitial keyword variants. 24h `url_harvest_cache` table fronts the cascade. `DestinationClass` extended 4 → 10 values. Live smoke captured the Fanplace link previously hidden behind tapforallmylinks.com's 2-step "Sensitive Content / Open link" gate. 192 pytest. tsc 0. ✅ **Profile noise filter retroactive cleanup + specific platform identification + AccountRow + banner foundation** (sync 16, 2026-04-26): T16 = soft-deleted 30 stale noise profile rows across all creators (`*.api.linkme.global`, `*.cloudfront.net`, `*.page.link`, empty-path homepages, legal/footer paths, plus a Phase-1.2 follow-up for `music.link.me`); resolver `_classify_and_enrich` now calls `is_noise_url(canon)` immediately after `visited_canonical.add` — drops noise URLs before they become `DiscoveredUrl` rows; `_commit_v2` URL-keyed dedup pass before RPC; PLATFORMS dict extended (reddit/threads/bluesky/snapchat); unified `lucide Link` clip icon for all aggregator types (linktree/beacons/custom_domain); All Destinations panel moved below tabs in collapsed `<details>`. T17 = Postgres `platform` enum extended +19 values to ~37 total (migration `20260426040000_add_platform_values_specific_aggregators_and_monetization`); Pydantic `Platform` Literal in `scripts/schemas.py` + `test_schemas.py` extended to match (caught and fixed by implementer when discovery runs failed pre-commit); gazetteer rewritten with 13 specific host→platform rules — 6 older generic rules removed so new specifics win (e.g. `tapforallmylinks.com` → `tapforallmylinks`, `cash.app` → `cashapp`, `link.me` → `link_me`, `app.fanfix.io` → `fanfix`, `venmo.com` → `venmo`); `src/lib/platforms.ts` PLATFORMS dict gained 13+ new entries with proper Si* icons (Spotify/Substack/Discord/WhatsApp/Kofi/BuyMeACoffee verified in react-icons 5.6.0) + lucide fallbacks for Cash App/Venmo/Fanfix; `resolvePlatform(platform, url)` now derives the URL hostname as the display label when `platform='custom_domain'` and no specific match → custom funnel domains show as e.g. "ariaswan.com" instead of generic "Custom Domain"; `HOST_PLATFORM_MAP` extended with all new aggregator/monetization hosts so URL-host inference works for legacy `platform='other'` rows; AccountRow layout restructured per Simon's UX call — Column 1 (180px) = `[icon] [Platform Name]`, Column 2 (280px) = handle + display_name secondary line; 11 backfill UPDATEs converted existing rows from `other`/`custom_domain` to specific platforms (tapforallmylinks=3, link_me=1, fanfix=1, cashapp=1, venmo=1, snapchat=2, reddit=1, spotify=1); migration `20260426050000_creator_cover_and_banner` added 3 `creators` columns (`cover_image_url`, `banner_url`, `override_avatar_url`); new `<BannerWithFallback>` client component on creator HQ; both creator HQ + grid card now prefer `override_avatar_url`. All 5 creators re-discovered cleanly (Esmae, Aria, Kira, Natalie, Valentina; ~$1 Apify spend). 227 pytest. tsc 0. ✅ **Duplicate prevention hardened with regression-test ring + Gemini-enriched watchdog** (sync 17, 2026-04-26): T18 = centralized `_normalize_handle(handle, platform)` helper in `scripts/discover_creator.py` strips leading `@` and lowercases for 35 case-insensitive platforms (`_CASE_INSENSITIVE_PLATFORMS` frozenset) — applied to seed account + every enriched-context append + every discovered-only append + funnel_edges (from_handle + to_handle); `_r` (TikTok client refresh) + `lang` (locale) added to `_TRACKING_PARAMS` in `scripts/pipeline/canonicalize.py`; new `_classify_linkme_redirector` runs FIRST in `pipeline.classifier.classify()` (before gazetteer) and parses `?sensitiveLinkLabel=OF/Fanvue/Fanfix/Fanplace/Patreon` to return `(<platform>, monetization)` at confidence 1.0; data migration soft-deleted 5 duplicate profile rows (Kira's TT case-variants + Natalie's YT + Twitter case dupes) and normalized 7 active rows (one-time tombstone of 5 soft-deleted handles with `__deleted_<uuid>` suffix to free unique-key slots); reclassified Kira's `visit.link.me/kirapregiato?sensitiveLinkLabel=OF` row from `(linktree, link_in_bio)` → `(onlyfans, monetization)` retroactively. T19 = new `scripts/tests/test_commit_v2_dedup.py` (13 tests covering every duplicate-prone failure mode through `_commit_v2`); new `scripts/tests/test_platform_enum_drift.py` (2 static tests locking in all 19 T17 enum additions + 18 pre-T17 values); migration `20260426060000_new_platform_watchdog_view` surfaces `platform='other'` rows grouped by host. T20 = migration `20260426070000_classifier_llm_guesses_enriched_metadata` adds 4 nullable TEXT columns (`suggested_label`, `suggested_slug`, `description`, `icon_category`); LLM prompt rewritten to return enriched suggestion fields; `_classify_via_llm` now returns 5-tuple; migration `20260426080000_watchdog_view_with_llm_suggestions` REPLACES the T19 view with a CTE-based join to `classifier_llm_guesses` so VAs see Gemini's per-host suggestion alongside the row (one-click ratification). 249 pytest + 1 skip. tsc 0.
   ...
   2026-05-01 sync 22 hardening: the harvester now preserves Tier 1 static anchors whenever Tier 2/headless escalation runs, then merges headless captures instead of replacing the static list. If headless returns empty, static links still flow to classification and persistence. This fixed the Shirley Pun Linktree case where `spa_hydration` triggered Tier 2, headless returned no URLs, and all visible Linktree bio links were dropped. `canonicalize_url()` also strips the Instagram `hl` locale param. Locked by `test_tier2_fires_when_signals_tripped`, `test_tier2_empty_falls_back_to_tier1_anchors`, and `test_strips_instagram_hl_param`.
9. 🟡 **Phase 2 scraping (manual-trigger + observability):** ✅ `scripts/scrape_content.py` CLI ships IG + TikTok ingestion (default last 30 days; 90-day run completed 2026-04-27) via batched Apify calls, `commit_scrape_result` RPC for transactional `scraped_content` + `content_metrics_snapshots` writes, `flag_outliers` per-profile, `profile_metrics_snapshots` daily row. Sync 19 hardening: fetchers stream all Apify dataset items via `iterate_items()`, IG/TT actor failures are isolated per platform, successful commits update `profiles.last_scraped_at`, `/scraped-content` and platform outlier pages are live, `scripts/validate_scraped_content.py` applies deterministic quality flags, `scripts/judge_suspicious_content.py` is the manual Gemini judge for suspicious rows, `schemas/social_post.schema.json` backs Apify results-checker config, and `scripts/extract_audio_trends.py` links repeated audio signatures into `trends`. 2026-04-28 follow-up: UI surfaces view-count coverage, IG static-post views are treated as unavailable instead of meaningful zeroes, `profile_metrics_snapshots.median_views` now ignores zero/missing view rows, content lists gained sorts/filters over stored Phase 2 metrics, and a deterministic `copyPriorityScore` ranks rows for manual inspiration review. `scripts/refresh_profile_avatars.py` uses profile-only/one-post actor inputs to refresh IG/TT avatar + profile metrics with limited Apify spend; live run refreshed 16/18 active IG/TT profile avatars. `scripts/scrape_instagram_highlights.py` imports dedicated-actor IG highlights into `scraped_content` as `post_type='story_highlight'`; live import added 139 story-highlight rows from the already-paid dataset `6XHq1bewmAcEjDVfc`. Current live content count after sync 21: 788 rows (`image` 24, `carousel` 85, `reel` 245, `tiktok_video` 295, `story_highlight` 139). `scrape_runs` migration exists locally but remains pending live apply (§4.3). Selection by `--creator-id` (repeatable) | `--tracking-type` | `--profile-id` (escape hatch). Specs/plans: `docs/superpowers/specs/2026-04-27-content-scraper-v1-design.md`, `docs/superpowers/plans/2026-04-27-content-scraper-v1.md`, `docs/superpowers/plans/2026-04-27-scraping-followup-without-cron.md`, `docs/superpowers/plans/2026-04-28-winning-content-intelligence.md`. **Deferred by Simon 2026-04-27:** GitHub Actions 12h cron / automatic scheduled scraping.

   2026-05-01 sync 22 live scrape note: Shirley Pun was repaired and scraped after discovery. The 90-day scrape loaded 128 TikToks + 6 Instagram posts; Simon requested an exception for full Instagram history, so IG was re-run with `--limit-days 3650`, loading 24 total IG rows from 2024-09-09 through 2026-04-28. Shirley now has 152 scraped rows (24 IG, 128 TT). Current live content library after this backfill is 940 rows. A repeat-audio refresh ran after the backfill and linked no additional IG posts.
10. 🟡 **Phase 2 trends:** repeated-audio extraction exists via `scripts/extract_audio_trends.py` (reads `platform_metrics.audio.signature`, upserts `trends`, sets `scraped_content.trend_id` when usage >= 2). `/trends` is now a live Audio Trends table with search/min-usage/min-creators/sort, usage count, cross-creator count, and distinct creator count; links back to filtered scraped content. Broader trend signals remain Phase 2–3 work.
...
| Instagram/TikTok CDN avatar expiry + partial avatar coverage | `AvatarWithFallback`, `AccountCard`, `AccountRow`, `src/lib/db/queries.ts`, `src/app/api/avatar/route.ts`, `scripts/refresh_profile_avatars.py` | Avatar URLs from platform CDNs are signed/expiring. 2026-04-28 mitigation: platform-account and creator reads now fall back to the latest avatar URL found in recent `scraped_content.raw_apify_payload` / `platform_metrics.author_avatar_url`; UI loads allowed platform-CDN avatars through `/api/avatar`; scraper writes fresh avatar URLs back to `profiles.avatar_url` on successful runs. Live refresh 2026-04-28: 18 active IG/TT profiles, 16 with `profiles.avatar_url`; still missing from Apify profile detail: IG `kirapregiato_backup`, IG `ellableu.fit`. | Re-run `python3 scripts/refresh_profile_avatars.py --workspace-id <ws> --platform both --all --limit 25` when platform CDN signatures expire or after new accounts are added. Long-term: cache profile avatars into Supabase Storage so the app owns stable image URLs. |
...
| **Discovery highlights branch still shelved; content-library highlight import is live** | `scripts/fetchers/instagram_highlights.py`, `scripts/pipeline/resolver.py`, `scripts/scrape_instagram_highlights.py` | Discovery-side highlights (`DISCOVERY_HIGHLIGHTS_ENABLED`) remain off because `apify/instagram-scraper resultsType=stories` is unreliable: 2026-04-28 diagnostic on @esmaecursed returned normal feed/reel records, not highlight records. The content-library use case now has a separate cheaper path: `seemuapps/instagram-highlights-scraper` with `usernames` input returned 14 highlight reels and 139 stories across current IG profiles; importer writes them to `scraped_content` as `story_highlight` rows through `commit_scrape_result`. | Keep discovery highlights off until there is evidence that highlight-only CTAs materially affect creator discovery. For content assets, use `scripts/scrape_instagram_highlights.py --workspace-id <ws> --limit <n>` or `--dataset-id <id>` to reuse paid datasets. Future UI: add creator-level Highlights tab and classify highlight CTAs. |
...
| **Apify credit exhaustion blocks discovery runs** (sync 22, 2026-05-01) | Discovery fetchers / Apify actor launch | The latest discovery retry for `@egirlyui` failed before actor execution because Apify reported remaining usage of only `$0.002231` and refused the job. This is not an app-code failure; the worker correctly marks the discovery run failed when the external actor cannot start. | Add Apify credits, then retry the failed discovery run. Longer-term option: add a cheap preflight/account-balance warning before dispatch if Apify exposes a reliable endpoint through the current client. |
...
| **IG static posts still lack true view counts** (sync 18, mitigated 2026-04-28) | `scripts/content_scraper/normalizer.py::instagram_to_normalized` + content UI | apify/instagram-scraper does not populate `videoPlayCount` / `videoViewCount` for static posts (carousels, photos). 2026-04-28 mitigation: UI labels those rows as view-unavailable, content/outlier pages show view coverage, `PlatformMetrics.view_count_source` records the actor field used on new scrapes, and `profile_metrics_snapshots.median_views` ignores zero/missing rows so static posts no longer collapse profile medians. Historical rows need a re-scrape to get `view_count_source`. | Phase 3 content analysis or a richer data source can inject reach/impressions for IG static content. Until then, do not compare static IG posts by views. |
...
2. **URL canonicalization** — `canonicalize_url()` strips tracking params (utm_*, fbclid, igsh, _r, lang, hl, l_, t, s, aff, ref_id, others), lowercases hosts, normalizes twitter.com→x.com, lowercases first path segment for known social platforms (`_SOCIAL_HANDLE_HOSTS`).
...
### Harvester completeness

7. **Tier 2 headless harvest cannot erase Tier 1 static anchors** — `harvest_urls()` always carries static anchors forward when escalation signals trigger Tier 2. Headless captures are merged into the Tier 1 list; an empty headless result falls back to the static anchors instead of returning zero rows. This protects Linktree/Beacons-style pages where visible server-rendered links coexist with SPA markers that trigger headless escalation. Locked by `tests/harvester/test_orchestrator.py::test_tier2_fires_when_signals_tripped` and `test_tier2_empty_falls_back_to_tier1_anchors`.

### Cross-creator dedup

8. **`run_cross_workspace_merge_pass`** runs at the end of every bulk import. Reads `profile_destination_links` for shared monetization/aggregator URLs across creators and raises merge candidates at confidence ≥ 0.7 (banner UI), auto-merges at 1.0 (direct link chain).

### Watchdog

9. **`new_platform_watchdog` SQL view** — list every active profile row with `platform='other'` grouped by URL host, **joined via CTE to `classifier_llm_guesses` (T20 / sync 17, migration `20260426080000`) to surface Gemini's enriched suggestion (`suggested_label`, `suggested_slug`, `description`, `icon_category`) per host**. Run periodically:
   ```sql
...
| TikTok `?_r=1`, locale `?lang=en`, and Instagram `?hl=en` query params surviving canonicalize | `test_strips_tiktok_r_param`, `test_strips_lang_param`, `test_strips_instagram_hl_param` |
| Tier 2 headless escalation returns empty and erases visible Tier 1 static links | `test_tier2_empty_falls_back_to_tier1_anchors` |
...
- 2026-04-28 (sync 21): **Winning-content triage, avatar refresh, and IG highlights import.** Added `docs/superpowers/plans/2026-04-28-winning-content-intelligence.md` to split Phase 2-safe filters/sorts from Phase 3 label/analyzer work. Content surfaces now expose known/missing view coverage, format filters, pinned/sponsored scopes, copy-priority sort, likes/comments/shares/saves sorts, repeat-audio post + distinct-creator counts, min-creator trend filtering, and high-priority summary cards. `profile_metrics_snapshots.median_views` now ignores zero/missing view rows so IG static posts do not collapse account medians. Creator/account profile photos now use `profiles.avatar_url` plus read-time fallback from recent scraped payloads and load allowed platform-CDN images through `/api/avatar`; content scrapes persist `platform_metrics.author_avatar_url` and refresh `profiles.avatar_url` when available. Live limited-profile refresh updated 16/18 active IG/TT avatars; only IG `kirapregiato_backup` and IG `ellableu.fit` still returned no avatar from Apify details. Official `apify/instagram-scraper resultsType=stories` was re-diagnosed and still unreliable for highlights, so `scripts/scrape_instagram_highlights.py` was added for the dedicated `seemuapps/instagram-highlights-scraper` actor and imported 139 `story_highlight` rows from dataset `6XHq1bewmAcEjDVfc`. Current live content library: 788 rows (`image` 24, `carousel` 85, `reel` 245, `tiktok_video` 295, `story_highlight` 139). Primary sidebar now only exposes working product surfaces. Verification: targeted content scraper/highlight tests 37/37, full Python tests 302/302 + 1 skip, `npm run typecheck` 0, `npm run lint` 0, `npm run build` 0, localhost route smokes 200 for `/`, `/creators`, `/platforms/instagram/accounts`, `/scraped-content`.
- 2026-05-01 (sync 22): **Harvester hardening, Shirley Pun backfill, and Codex sync policy.** Commit `ab2ee0b` fixed the Universal URL Harvester regression where Tier 2/headless escalation could replace Tier 1 static anchors with an empty result. `scripts/harvester/orchestrator.py` now preserves static anchors, merges headless captures, and falls back to Tier 1 links if headless captures nothing; `scripts/pipeline/canonicalize.py` strips the Instagram `hl` locale query param; `scripts/worker_ctl.sh status` now uses `launchctl print gui/$(id -u)/com.thehub.worker` so launchd status reflects the actual worker. Live repair: Shirley Pun discovery now persists the Linktree child links and primary IG profile correctly (20 profile rows, 24 destination rows after repair). Content scrape: 90-day scrape loaded 128 TikToks + 6 IG posts; Simon requested a creator-specific full-history exception, so IG was re-run with `--limit-days 3650`, resulting in 24 IG rows from 2024-09-09 through 2026-04-28 and 152 Shirley rows total. Current live scraped-content library after this backfill is 940 rows. `scripts/extract_audio_trends.py --min-usage 2` was re-run and linked no additional IG backfill posts. Latest discovery retry for `@egirlyui` failed because Apify credits were exhausted (`$0.002231` remaining); no code fix needed until credits are topped up and the run is retried. Worker is launchd-managed and running. Codex handoff updated so sync-project-state/session notes run after material code, data, scraper, or operational changes. Verification: full Python tests 304 passed + 1 skip, `npm run typecheck` 0, `npm run lint` 0, `npm run build` 0, sorted route smokes 200.
