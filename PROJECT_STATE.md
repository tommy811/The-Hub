...
Last synced: 2026-04-25 (sync 14 — recursive funnel resolver shipped + Highlights v1 code shipped/runtime gated on IG auth)
...
### 4.1 Currently live in Supabase (23 tables)
...
- `profile_destination_links` — profile_id (FK→profiles), canonical_url, destination_class (`monetization|aggregator|social|other`), workspace_id, created_at. PK (profile_id, canonical_url). Persistent reverse index driving cross-workspace identity dedup. RLS: `is_workspace_member`.
...
| `commit_discovery_result` | (p_run_id, p_creator_data jsonb, p_accounts jsonb, p_funnel_edges jsonb, p_discovered_urls jsonb DEFAULT '[]', p_bulk_import_id uuid DEFAULT NULL) → jsonb | Transactional v2: enriches creator (source-aware — `manual_add` only union-merges known_usernames), upserts profiles, inserts funnel edges, writes `profile_destination_links`, bumps `bulk_imports.seeds_committed`, marks run completed. Returns `{creator_id, accounts_upserted, merge_candidates_raised, urls_recorded}` |
...
| `/creators/[slug]` | `(dashboard)/creators/[slug]/page.tsx` | 🟡 Partial — Brand HQ surface: stats strip, AvatarWithFallback, network sections with brand icons, Add Account + Re-run Discovery in header, **DiscoveryProgress live bar** during in-flight runs. Bio + platform identity stripped from header (account-level data lives only in the network rows). Content/Branding/Funnel tabs are coming-soon stubs. |
