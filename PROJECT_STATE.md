...
| `bulk_import_creator` | (p_handle, p_platform_hint, p_tracking_type, p_tags, p_user_id, p_workspace_id, p_bulk_import_id uuid DEFAULT NULL) → jsonb | Creates creator + primary profile + pending discovery_run; when p_bulk_import_id is NULL, creates a new `bulk_imports` row. Returns `{bulk_import_id, creator_id, run_id}` |
