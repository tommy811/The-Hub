...
Last synced: 2026-04-26 (sync 16 — profile noise filter retroactive cleanup + specific platform identification + AccountRow + banner foundation)
...
| **Pydantic `Platform` Literal mirrors Postgres `platform` enum manually** (sync 16, 2026-04-26) | `scripts/schemas.py` ↔ Postgres `platform` enum | `scripts/schemas.py` declares `Platform = Literal[...]` with the full list of values; the live Postgres enum is the source of truth. They are kept in sync by hand. T17's enum extension was caught only because the implementer's first discovery test run failed pre-commit on a Pydantic ValidationError — without that pre-commit guard, future enum extensions could silently break discovery (Postgres accepts the value, Pydantic rejects on the way back). | Future work: CI test that diffs `scripts/schemas.py::Platform.__args__` against `SELECT enumlabel FROM pg_enum WHERE enumtypid = 'platform'::regtype` and fails the build on mismatch. Easy to write; not yet prioritized. |
| **`visit.link.me/.../sensitiveLinkLabel=OF` URLs dropped by eTLD+1 same-domain filter** (T15 known limitation, still open in sync 16) | `scripts/harvester/orchestrator.py` — same-host filter | The same-host self-link drop (introduced in sync 15 commit `b632b81` to stop aggregator footers surfacing as their own destinations) over-eagerly drops some link.me redirector URLs whose query string carries a telling marker (e.g. `sensitiveLinkLabel=OF`). The redirector itself is a link.me URL, but its target is OnlyFans — useful signal we're losing. | Option (b) from sync 15 — keep URLs with telling query markers (`sensitiveLinkLabel`, similar gates) — not yet implemented. Audit cost vs. benefit when the slice of OF-adjacent creators relying on this redirector pattern materially affects discovery completeness. |
